...
router.put('/profile', authenticateToken, (req, res) => {
  const { nickname, title, title_color } = req.body;
  const user = db.prepare('SELECT * FROM users WHERE id = ?').get(req.user.id);
  if (!user) return res.status(404).json({ error: '用户不存在' });
  if (user.status === 'banned') return res.status(403).json({ error: '账号已被封禁' });
  ...
  db.prepare('UPDATE users SET nickname = ?, title = ?, title_color = ? WHERE id = ?')
    .run(
      nickname || req.user.username,
      title !== undefined ? (title || null) : user.title,
      title_color !== undefined ? title_color : (user.title_color || '#00e5ff'),
      req.user.id
    );
